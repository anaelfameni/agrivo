...
> **Document de référence officiel du projet** — version du 6 juillet 2026, rédigé par Anael (chef de projet).
> Il remplace la partie « état du produit » de `AGRIVO_Plan_Fonctionnement_et_Equipe.pdf` (6 juillet, matin)
> et intègre TOUT ce qui a changé depuis (sessions 14 à 20). Un nouvel arrivant doit pouvoir comprendre
> le projet en lisant ce seul document. Confidentiel équipe : Anael · Christ · Gaddiel · Domy.
> Jury : **samedi 11 juillet 2026, CSCTICAO** · Produit : **https://agrivo-io.vercel.app** (v1.1.0).
...
La clé Gemini est posée et testée en production (session 19). Trois features tournent **en LIVE** sur
https://agrivo-io.vercel.app (modèle `gemini-2.5-flash`) :
...
- **Preuves de méthode** (le jury évalue le pipeline, pas juste la démo) : `SPECS.md` (user stories +
  ...
  **CI GitHub Actions** (lint + types + tests + build à chaque push), **32 tests Vitest**, tags
  `v1.0.0` / `v1.1.0`.
...
- **Dashboard** : bonjour Amadou, 4 KPI, **import & audit du registre**, recherche, dernières
...
- **Parcours « Nouvelle vérification »** (golden path, 6 étapes) : 1. Sélection coopérative →
  ...
  6. **Valorisation** (dossier de conformité, primes, partage exportateur).
...
| **Anael** | Relire le deck v3 (`AGRIVO_Pitch_Vibeathon2026.pptx`), action Vercel domaines (2 min : Project agrivo → Settings → Domains → Add `agrivo-io.vercel.app`), répétition générale sur PROD vendredi 10 (P9), chrono pitch 5:00. | Mer–ven |
...
4. ☐ Équipe : flashcards + réponses aux questions pièges (`GUIDE_DEMO_JURY.md`, chacun).
5. ☐ Vendredi 10 : **P9 — répétition générale sur PROD** (déroulé 7 min chronométré, scan QR depuis un téléphone, vidéo plan B en local ET sur clé USB).
6. ☐ Samedi 11 : jury au CSCTICAO — aucun changement de code après le gel.
...
- Compte démo coopérative : `client@test.com` / `123client123` (Amadou, Coop. de Soubré) — bouton « Entrer avec le compte de démonstration ».
...
- Documents du repo : `CLAUDE.md` (mémoire de travail) · `PLAN_REORIENTATION_AGRIVO.md` (pivot) ·
  ...
  `SPECS.md` · `ARCHITECTURE.md` · `CHANGELOG.md` · `SUPPORTS_PIVOT_P6.md` (contenus deck).

*AGRIVO — document interne équipe · 6 juillet 2026 · rédigé par Anael (chef de projet).*
