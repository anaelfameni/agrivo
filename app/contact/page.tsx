...
export default function ContactPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (form.nom.trim().length < 2) er.nom = "Indiquez votre nom.";
    if (!EMAIL_RE.test(form.email.trim())) er.email = "E-mail invalide.";
    if (form.message.trim().length < 10) er.message = "Votre message est un peu court.";
    setErrors(er);
  ...
  return (
    <div className="min-h-dvh bg-ivory text-forest-950">
      ...
      <main className="mx-auto max-w-5xl px-6 pb-24 pt-16 md:px-8 md:pt-20">
        ...
        <h1 className="mt-3 font-display text-4xl text-forest-950">Parlons de votre mise en conformité.</h1>
        <p className="mt-3 max-w-xl text-stone-600">
          Une question produit, une démonstration pour votre coopérative, un partenariat ? Écrivez-nous — nous
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
          <div className="rounded-2xl border border-black/[0.06] bg-white p-6 shadow-[0_1px_2px_rgba(10,31,20,0.04)] md:p-8">
            {sent ? (
              <motion.div initial={reduce ? { opacity: 0 } : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center py-8 text-center">
                ...
                <h2 className="mt-4 font-display text-2xl text-forest-950">Message envoyé</h2>
              ...
              <form onSubmit={submit} noValidate className="flex flex-col gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input value={form.organisation} onChange={set("organisation")} className={inputCls} placeholder="Coopérative, exportateur…" />
                  </label>
                </div>
                <label className="flex flex-col gap-1.5">
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <textarea value={form.message} onChange={set("message")} rows={5} className={`${inputCls} resize-y`} placeholder="Décrivez votre besoin…" />
                ...
                <button type="submit" disabled={pending} className="inline-flex items-center justify-center gap-2 rounded-full bg-green-signal px-6 py-3 text-sm font-semibold text-white shadow-[0_14px_34px_-12px_rgba(22,163,74,0.75)] transition-[filter] hover:brightness-105 disabled:opacity-70">
                  {pending ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Envoyer le message
                </button>
              </form>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            <ContactCard Icon={Building2} title="Commercial" desc="Démonstration, tarifs, déploiement pour votre coopérative ou en tant qu'exportateur." email="commercial@agrivo.example" />
            <ContactCard Icon={MessageSquare} title="Support" desc="Aide à l'utilisation, questions techniques, accès à votre compte." email="support@agrivo.example" />
            <div className="rounded-2xl border border-black/[0.06] bg-white p-5 text-sm text-stone-600">
              <div className="flex items-center gap-2 font-medium text-forest-950">
                <Mail size={16} className="text-green-signal" /> Réponse sous 48 h ouvrées
              </div>
              <p className="mt-2">Les adresses ci-dessus sont des exemples de démonstration, à remplacer par vos coordonnées réelles.</p>
            </div>
          </div>
        </div>
      </main>
