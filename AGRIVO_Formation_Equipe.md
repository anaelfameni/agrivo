...
compte démo coopérative `client@test.com / 123client123` (Amadou, Coopérative Agricole de Soubré) ·
...
| 0:30 | **Golden path sur MOBILE** (3 min) : consentement → scan carte (QR/Gemini) → cartographie GPS réelle → verdict satellite → certificat → valorisation | Christ | Le GPS et la caméra sont RÉELS sur le terrain |
...
| 4:45 | **Vue exportateur** (1 min 30) : carte du portefeuille, export GeoJSON TRACES NT, copilote IA (« Quelles parcelles présentent un risque dans la région de Soubré ? ») | Anael/Gaddiel | Le côté acheteur est déjà servi |
